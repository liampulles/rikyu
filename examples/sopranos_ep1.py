with project("/media/liam/Additional 2/Encodes/Sopranos1") as proj:
    vap = vapoursynth_template_from("/media/liam/Additional 2/Encodes/Sopranos1/Vapoursynth/episode.vpy")
    # TODO: Rerun for each disc, then comment out
    proj.rip_from_folder("/media/liam/SOPRANOS_SEASON1_DISC1/")
        .title(1, seq("episode", 1))
        .title(3, seq("episode", 2))
        .title(4, seq("episode", 3))
        .title(5, uniq("making of"))
    # TODO: Work on episodes
    ep_pipe = proj.pipeline([
    ...
    ep_pipe.apply_on_seq("episode",range(1,14))
